# 37. Ruweisat Ridge - First Assault
## SITUATION REPORT
**Date**: July 1-3, 1942 (multi-day battle, scenario represents climactic assault)
**Location**: Ruweisat Ridge, central Alamein line

New Zealand and Indian forces attack Axis-held Ruweisat Ridge - key terrain dominating central Alamein line. Series of assaults and counterassaults over 3 days.

Part of First Alamein operation.
## THE BATTLE
New Zealand and Indian forces attack Axis-held Ruweisat Ridge - key terrain dominating central Alamein line. Series of assaults and counterassaults over 3 days.

**British/NZ Objective**: Capture ridge (control crest)

**Axis Objective**: Hold ridge OR counterattack if lost
## THE BATTLEFIELD
**Table Size**: 8' × 4'

**Terrain**:
- **terrain**: Low ridge with defensive positions
- **terrain**: open approaches
- **terrain**: good fields of fire

**Special Battlefield Rules**:
- Ridge terrain (elevation advantage for defenders)
- Mixed Axis defense (German/Italian cooperation)
- Multiple assaults (can represent successive attacks over 3 days)

---

## OBJECTIVES
**Victory Type**: mixed

**British/NZ Victory**: Capture ridge (control crest)

**Axis Victory**: Hold ridge OR counterattack if lost

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British/NZ**: British/NZ deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Ridge terrain (elevation advantage for defenders)
- Mixed Axis defense (German/Italian cooperation)
- Multiple assaults (can represent successive attacks over 3 days)

**Turn Limit**: 8 turns

## FORCES

### BRITISH/NZ FORCES
**Nation**: British/Nz
**Points Budget**: 1350
**Total Battle Rating**: 21

**Units**:
- 21x British Infantry Battalion (veteran) - 3360 pts, BR: 21 [2 battalion, 650 men (~21 platoons)]

### AXIS FORCES
**Nation**: Axis
**Points Budget**: 1350
**Total Battle Rating**: 33

**Units**:
- 13x German Infantry Battalion (veteran) - 2080 pts, BR: 13 [1 battalion, ~400 men (assumed, ~13 platoons)]
- 13x Italian Infantry Battalion (veteran) - 2080 pts, BR: 13 [1 battalion, ~400 men (assumed, ~13 platoons)]
- 11x tanks (veteran) - 550 pts, BR: 3 [Informal range]
- 4x PAK 40 75mm AT guns 6-8 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]
- 4x 47mm Italian AT guns 6 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]
- 4x 105mm leFH 18 artillery 4-6 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]
- 4x 75mm Italian field guns 4 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
