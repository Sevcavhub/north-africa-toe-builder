# 43. Kidney Ridge Probe
## SITUATION REPORT
**Date**: July 22, 1942, 10:00
**Location**: Kidney Ridge (later famous in Second Alamein)

British reconnaissance in force toward Kidney Ridge. Probing Axis defenses in preparation for possible offensive.

Part of First Alamein operation.
## THE BATTLE
British reconnaissance in force toward Kidney Ridge. Probing Axis defenses in preparation for possible offensive.

**British Objective**: Probe defenses, gather intelligence (recon objectives)

**Axis Objective**: Hold positions, prevent intelligence gathering
## THE BATTLEFIELD
**Table Size**: 6' × 4'

**Terrain**:
- **terrain**: Kidney Ridge area with defensive positions

**Special Battlefield Rules**:
- Reconnaissance mission (British not seeking decisive battle)
- Limited engagement (both sides avoiding heavy casualties)
- Intelligence gathering (British earn points for identifying Axis positions)

---

## OBJECTIVES
**Victory Type**: mixed

**British Victory**: Probe defenses, gather intelligence (recon objectives)

**Axis Victory**: Hold positions, prevent intelligence gathering

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British**: British deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Reconnaissance mission (British not seeking decisive battle)
- Limited engagement (both sides avoiding heavy casualties)
- Intelligence gathering (British earn points for identifying Axis positions)

**Turn Limit**: 8 turns

## FORCES

### BRITISH FORCES
**Nation**: British
**Points Budget**: 700
**Total Battle Rating**: 35


### AXIS FORCES
**Nation**: Axis
**Points Budget**: 700
**Total Battle Rating**: 35


## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
