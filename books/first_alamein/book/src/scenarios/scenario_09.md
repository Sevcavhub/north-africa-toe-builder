# 44. Alam el Onsol - Australian Night Attack
## SITUATION REPORT
**Date**: July 26-27, 1942, 22:00-04:00
**Location**: Alam el Onsol, northern sector

9th Australian Division launches night assault on Axis positions at Alam el Onsol. Final major attack of First Alamein battle. Australian expertise in night operations on full display.

Part of First Alamein operation.
## THE BATTLE
9th Australian Division launches night assault on Axis positions at Alam el Onsol. Final major attack of First Alamein battle. Australian expertise in night operations on full display.

**Australian Objective**: Capture positions, advance salient

**Axis Objective**: Hold positions OR counterattack at dawn
## THE BATTLEFIELD
**Table Size**: 8' × 4'

**Terrain**:
- **terrain**: Defensive positions with minefields
- **terrain**: barbed wire

**Special Battlefield Rules**:
- Night assault (Australian specialty)
- Australian infantry excellence (high close-combat, high morale)
- Minefield breaching (engineers clearing paths)
- Dawn counterattack (Axis tanks available at sunrise)

---

## OBJECTIVES
**Victory Type**: mixed

**Australian Victory**: Capture positions, advance salient

**Axis Victory**: Hold positions OR counterattack at dawn

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**Australian**: Australian deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Night assault (Australian specialty)
- Australian infantry excellence (high close-combat, high morale)
- Minefield breaching (engineers clearing paths)
- Dawn counterattack (Axis tanks available at sunrise)

**Turn Limit**: 8 turns

## FORCES

### AUSTRALIAN FORCES
**Nation**: Australian
**Points Budget**: 1100
**Total Battle Rating**: 55

**Units**:
- 1x Mixed Force (veteran) - 1100 pts, BR: 55 [2 battalions 9th Australian Division (600-700 men), engineer support, artillery preparation...]

### AXIS FORCES
**Nation**: Axis
**Points Budget**: 1100
**Total Battle Rating**: 55

**Units**:
- 1x Mixed Force (veteran) - 1100 pts, BR: 55 [Mixed German/Italian defenders (1 battalion combined, defensive positions, minefields)...]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
