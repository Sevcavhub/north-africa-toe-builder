# 18. Gazala Pursuit
## SITUATION REPORT
**Date**: December 5-7, 1941 (representing pursuit phase)
**Location**: West of Gazala
**Scale**: Battalion-level engagement

British forces pursue withdrawing Axis forces westward. Germans conduct fighting withdrawal, delaying British while preserving strength. Mobile warfare with frequent skirmishes.

Part of Crusader operation.
## THE BATTLE
British forces pursue withdrawing Axis forces westward. Germans conduct fighting withdrawal, delaying British while preserving strength. Mobile warfare with frequent skirmishes.

**British Objective**: Destroy rearguard (60%+ casualties) or force withdrawal off table edge

**Axis Objective**: Delay pursuit (hold British 8+ turns) then withdraw
## THE BATTLEFIELD
**Table Size**: 6' × 4'

**Terrain**:
- **terrain**: Open desert with occasional defensive positions

**Special Battlefield Rules**:
- Fighting withdrawal (Axis can retreat off table edge)
- Pursuit (British enters from table edge)
- Delaying actions (Axis bonuses for slowing British)

---

## OBJECTIVES
**Victory Type**: mixed

**British Victory**: Destroy rearguard (60%+ casualties) or force withdrawal off table edge

**Axis Victory**: Delay pursuit (hold British 8+ turns) then withdraw

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British**: British deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Fighting withdrawal (Axis can retreat off table edge)
- Pursuit (British enters from table edge)
- Delaying actions (Axis bonuses for slowing British)

**Turn Limit**: 8 turns

## FORCES

### BRITISH FORCES
**Nation**: British
**Points Budget**: 900
**Total Battle Rating**: 12

**Units**:
- 24x tanks 20-24 Crusader/Honey (veteran) - 1200 pts, BR: 8 [2 squadron (~12/squadron assumed)]
- 3x Motorized Infantry Company (veteran) - 480 pts, BR: 3 [1 companies, ~90 men (assumed)]
- 4x RHA 25-pdr) (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]

### AXIS FORCES
**Nation**: Axis
**Points Budget**: 900
**Total Battle Rating**: 9

**Units**:
- 11x tanks (veteran) - 550 pts, BR: 3 [Informal range]
- 7x tanks (veteran) - 350 pts, BR: 2 [Informal range]
- 3x Motorized Infantry Company (veteran) - 480 pts, BR: 3 [1 companies, ~90 men (assumed)]
- 2x 88mm) (veteran) - 100 pts, BR: 1

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
