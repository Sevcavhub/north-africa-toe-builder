# 15. Rommel's Dash to the Wire
## SITUATION REPORT
**Date**: November 24-26, 1941 (representing multi-day action)
**Location**: Egyptian frontier, "the Wire"

Rommel's famous "dash to the wire" - personal leading of mobile kampfgruppe deep into British rear areas attempting to create panic and disrupt British logistics. Bold but ultimately unsuccessful raid.

Part of Crusader operation.
## THE BATTLE
Rommel's famous "dash to the wire" - personal leading of mobile kampfgruppe deep into British rear areas attempting to create panic and disrupt British logistics. Bold but ultimately unsuccessful raid.

**German Objective**: Raid objectives (destroy supply dumps, capture trucks, create chaos)

**British Objective**: Defend rear areas, delay raiding force
## THE BATTLEFIELD
**Table Size**: 6' × 4'

**Terrain**:
- **terrain**: Desert with British supply dumps
- **terrain**: vehicle parks
- **terrain**: scattered positions

**Special Battlefield Rules**:
- Raid scenario (German must hit objectives and withdraw)
- Scattered defenders (British start disorganized)
- Supply dumps (flammable/exploding objectives)
- Time limit (Germans must withdraw by turn 10)

---

## OBJECTIVES
**Victory Type**: mixed

**German Victory**: Raid objectives (destroy supply dumps, capture trucks, create chaos)

**British Victory**: Defend rear areas, delay raiding force

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**German**: German deploy within 12" of south table edge

**British**: British deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Raid scenario (German must hit objectives and withdraw)
- Scattered defenders (British start disorganized)
- Supply dumps (flammable/exploding objectives)
- Time limit (Germans must withdraw by turn 10)

**Turn Limit**: 8 turns

## FORCES

### GERMAN FORCES
**Nation**: German
**Points Budget**: 900
**Total Battle Rating**: 45

**Units**:
- 1x Mixed Force (veteran) - 900 pts, BR: 45 [Kampfgruppe (2 companies Panzer III/IV 18-20 tanks, 1 company motorized infantry, 1 battery artiller...]

### BRITISH FORCES
**Nation**: British
**Points Budget**: 900
**Total Battle Rating**: 45

**Units**:
- 1x Mixed Force (veteran) - 900 pts, BR: 45 [Scattered rear-area units (1 company infantry, 2-3 tanks, supply column, artillery battery), unprepa...]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
