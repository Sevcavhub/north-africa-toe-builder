# 26. Bir Hacheim - Breakout
## SITUATION REPORT
**Date**: June 10-11, 1942, 22:00-04:00 (night breakout)
**Location**: Bir Hacheim

Free French ordered to evacuate Bir Hacheim after 14-day siege. Night breakout through German/Italian encirclement. Heroic escape of majority of garrison.

Part of Gazala operation.
## THE BATTLE
Free French ordered to evacuate Bir Hacheim after 14-day siege. Night breakout through German/Italian encirclement. Heroic escape of majority of garrison.

**Free French Objective**: Break out (exit 50%+ force off table edge to safety)

**Axis Objective**: Prevent breakout (destroy/capture French garrison)
## THE BATTLEFIELD
**Table Size**: 6' × 4'

**Terrain**:
- **terrain**: Dark (night action)
- **terrain**: minefields (both friendly and enemy)
- **terrain**: scattered German positions

**Special Battlefield Rules**:
- Night breakout (limited visibility, infiltration)
- Minefield navigation (French know own fields, must avoid German fields)
- Escape and evasion (French using stealth and speed)
- German/Italian patrols (scattered blocking positions)

---

## OBJECTIVES
**Victory Type**: mixed

**Free French Victory**: Break out (exit 50%+ force off table edge to safety)

**Axis Victory**: Prevent breakout (destroy/capture French garrison)

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**Free French**: Free French deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Night breakout (limited visibility, infiltration)
- Minefield navigation (French know own fields, must avoid German fields)
- Escape and evasion (French using stealth and speed)
- German/Italian patrols (scattered blocking positions)

**Turn Limit**: 8 turns

## FORCES

### FREE FRENCH FORCES
**Nation**: Free French
**Points Budget**: 900
**Total Battle Rating**: 45

**Units**:
- 1x Mixed Force (veteran) - 900 pts, BR: 45 [Breakout force (remaining garrison 60-70% original strength, limited vehicles, wounded)...]

### AXIS FORCES
**Nation**: Axis
**Points Budget**: 900
**Total Battle Rating**: 64

**Units**:
- 90x men (veteran) - 4500 pts, BR: 30 [Informal range]
- 90x men (veteran) - 4500 pts, BR: 30 [Informal range]
- 6x Panzer III tanks (veteran) - 300 pts, BR: 2
- 2x 50mm PAK 38 (veteran) - 100 pts, BR: 1
- 2x Italian 47mm AT guns (veteran) - 100 pts, BR: 1

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
