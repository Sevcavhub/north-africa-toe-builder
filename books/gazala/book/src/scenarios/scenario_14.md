# 34. Pursuit to Mersa Matruh II
## SITUATION REPORT
**Date**: June 27, 1942, 14:00
**Location**: Mersa Matruh defensive positions

British attempt to hold Mersa Matruh defensive line. German forces probe defenses. British abandon position rather than risk encirclement.

Part of Gazala operation.
## THE BATTLE
British attempt to hold Mersa Matruh defensive line. German forces probe defenses. British abandon position rather than risk encirclement.

**British Objective**: Hold line OR withdraw if threatened with encirclement

**German Objective**: Breach defenses or force British withdrawal
## THE BATTLEFIELD
**Table Size**: 8' × 4'

**Terrain**:
- **terrain**: Coastal defensive position with minefields
- **terrain**: AT positions

**Special Battlefield Rules**:
- Hasty defenses (British unprepared positions)
- Probing attack (German reconnaissance in force)
- Withdrawal option (British can retreat to avoid trap)

---

## OBJECTIVES
**Victory Type**: mixed

**British Victory**: Hold line OR withdraw if threatened with encirclement

**German Victory**: Breach defenses or force British withdrawal

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British**: British deploy within 12" of south table edge

**German**: German deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Hasty defenses (British unprepared positions)
- Probing attack (German reconnaissance in force)
- Withdrawal option (British can retreat to avoid trap)

**Turn Limit**: 8 turns

## FORCES

### BRITISH FORCES
**Nation**: British
**Points Budget**: 1100
**Total Battle Rating**: 1

**Units**:
- 4x 25-pdr (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]

### GERMAN FORCES
**Nation**: German
**Points Budget**: 1100
**Total Battle Rating**: 2

**Units**:
- 4x 50mm PAK 38 AT guns 6 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]
- 4x 105mm leFH 18 artillery 4 guns (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
