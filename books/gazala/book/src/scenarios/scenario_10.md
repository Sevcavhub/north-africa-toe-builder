# 30. The Gazala Gallop - Running Fight
## SITUATION REPORT
**Date**: June 14, 1942, 10:00
**Location**: Coastal road east of Gazala
**Scale**: Battalion-level engagement

British forces stream eastward in retreat. German forces pursue, conducting running battles. British attempt to delay pursuit while preserving forces.

Part of Gazala operation.
## THE BATTLE
British forces stream eastward in retreat. German forces pursue, conducting running battles. British attempt to delay pursuit while preserving forces.

**British Objective**: Delay pursuit (hold 6 turns) then withdraw

**German Objective**: Destroy rearguard (60%+ casualties)
## THE BATTLEFIELD
**Table Size**: 6' × 4'

**Terrain**:
- **terrain**: Desert coastal area
- **terrain**: British moving east to west across table

**Special Battlefield Rules**:
- Fighting withdrawal (British can exit table edge)
- Aggressive pursuit (German bonuses for rapid advance)
- Delaying action (British buy time for main force)

---

## OBJECTIVES
**Victory Type**: mixed

**British Victory**: Delay pursuit (hold 6 turns) then withdraw

**German Victory**: Destroy rearguard (60%+ casualties)

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British**: British deploy within 12" of south table edge

**German**: German deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Fighting withdrawal (British can exit table edge)
- Aggressive pursuit (German bonuses for rapid advance)
- Delaying action (British buy time for main force)

**Turn Limit**: 8 turns

## FORCES

### BRITISH FORCES
**Nation**: British
**Points Budget**: 900
**Total Battle Rating**: 8

**Units**:
- 12x tanks (veteran) - 600 pts, BR: 4 [1 squadron (~12/squadron assumed)]
- 3x Motorized Infantry Company (veteran) - 480 pts, BR: 3 [1 companies, ~90 men (assumed)]
- 4x RHA 25-pdr (veteran) - 200 pts, BR: 1 [1 battery (4 guns per battery assumed)]

### GERMAN FORCES
**Nation**: German
**Points Budget**: 900
**Total Battle Rating**: 6

**Units**:
- 18x tanks (veteran) - 900 pts, BR: 6 [Informal range]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
