# 22. The Cauldron Forms
## SITUATION REPORT
**Date**: May 28, 1942, 10:00
**Location**: East of Bir Hacheim, "The Cauldron"
**Scale**: Battalion-level engagement

Rommel's forces trapped in "Cauldron" east of British minefields after left-hook maneuver. British minefields block retreat, British armor surrounds from east. Rommel in desperate situation but brilliantly turns tables.

Part of Gazala operation.
## THE BATTLE
Rommel's forces trapped in "Cauldron" east of British minefields after left-hook maneuver. British minefields block retreat, British armor surrounds from east. Rommel in desperate situation but brilliantly turns tables.

**British Objective**: Destroy trapped Axis force (70%+ casualties)

**Axis Objective**: Survive and break British attacks OR break through minefields
## THE BATTLEFIELD
**Table Size**: 8' × 6'

**Terrain**:
- **terrain**: Open desert with British minefields to west (blocking Axis escape)
- **terrain**: shallow depressions providing limited cover

**Special Battlefield Rules**:
- Surrounded (Axis starts in center, British attacks from multiple sides)
- Minefield barrier (Axis trapped against minefields initially)
- Desperate defense (Axis high stakes)
- British overconfidence (British expected easy victory)

---

## OBJECTIVES
**Victory Type**: mixed

**British Victory**: Destroy trapped Axis force (70%+ casualties)

**Axis Victory**: Survive and break British attacks OR break through minefields

**Draw**: Neither side achieves their objectives

## DEPLOYMENT
**British**: British deploy within 12" of south table edge

**Axis**: Axis deploy within 12" of north table edge

**Turn Order**: Roll off for initiative

## SPECIAL SCENARIO RULES
- Surrounded (Axis starts in center, British attacks from multiple sides)
- Minefield barrier (Axis trapped against minefields initially)
- Desperate defense (Axis high stakes)
- British overconfidence (British expected easy victory)

**Turn Limit**: 8 turns

## FORCES

### BRITISH FORCES
**Nation**: British
**Points Budget**: 1650
**Total Battle Rating**: 44

**Units**:
- 48x tanks 40-45 Grant/Crusader (veteran) - 2400 pts, BR: 16 [4 squadron (~12/squadron assumed)]
- 26x Infantry Battalion (veteran) - 4160 pts, BR: 26 [2 battalion, ~800 men (assumed, ~26 platoons)]
- 8x 25-pdr) (veteran) - 400 pts, BR: 2 [2 battery (4 guns per battery assumed)]

### AXIS FORCES
**Nation**: Axis
**Points Budget**: 1650
**Total Battle Rating**: 22

**Units**:
- 32x tanks (veteran) - 1600 pts, BR: 10 [Informal range]
- 17x M13/40 (veteran) - 850 pts, BR: 5 [2 companies]
- 6x 50mm PAK 38 (veteran) - 300 pts, BR: 2
- 3x 88mm FlaK 18 (veteran) - 150 pts, BR: 1
- 8x Italian 47mm AT guns (veteran) - 400 pts, BR: 2 [2 battery (4 guns per battery assumed)]
- 8x 105mm leFH 18 artillery) (veteran) - 400 pts, BR: 2 [2 battery (4 guns per battery assumed)]

## ALTERNATIVE FORCES
This scenario can be adapted for other battles or periods.

- Use units from adjacent quarters for force variation
- Adjust points budget for smaller/larger engagements
