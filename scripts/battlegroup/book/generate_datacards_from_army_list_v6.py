#!/usr/bin/env python3
"""
Generate BattleGroup datacards from a text army list (V6.1).

V6.1 changes (November 2025):
- Use V6.1 datacard generator with weapon fallback support
- Enables datacards for vehicles not in bg_reference_vehicles (e.g., Sherman Jumbo)
- Non-breaking: Preserves manual extraction priority from bg_reference_vehicles
...
3. Generates V6.1 format datacards with automatic weapon fallback
...
Usage:
    python generate_datacards_from_army_list_v6.py --input army_list.txt --output datacards/
    python generate_datacards_from_army_list_v6.py --equipment "Panzer III,Panzer IV,88mm FlaK" --output test/
"""
