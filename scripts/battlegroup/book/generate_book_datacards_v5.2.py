    ...
    def generate_datacard_markdown(self, equipment: Dict, experience: str = 'r') -> str:
        ...
        if not main_gun and row['reference_vehicle_id']:
            ...
            if ref_row:
                ...
                for i in range(1, 5):
                    weapon_col = f'weapon_{i}'
                    mount_col = f'mount_{i}'
                    ammo_col = f'ammo_{i}'

                    weapon_name = ref_row[weapon_col]
                    mount = ref_row[mount_col] or ''
                    ammo = ref_row[ammo_col]

                    if weapon_name and 'turret' in mount.lower() and weapon_name.upper() != 'MG':
        ...
        if not secondary and row['reference_vehicle_id']:
            ...
            if ref_row:
                ...
                for i in range(1, 5):
                    ...
                    weapon_name = ref_row[weapon_col]
                    mount = ref_row[mount_col] or 'Unknown'
                    weapon_ammo = ref_row[ammo_col]

                    # Get all weapons except the main gun we already extracted
                    # Include co-axial/bow MGs and other secondary armament
                    if weapon_name and weapon_name != main_gun and weapon_name.upper() != 'NONE':
                        secondary.append({
                            'name': weapon_name,
                            'mount': mount,
                            'ammo': weapon_ammo
                        })
                    elif weapon_name == main_gun and i > 1:
                        # Skip if this is the same as main gun (already captured)
                        continue
        ...
        # Set defaults for mount and ammo display (used in template)
