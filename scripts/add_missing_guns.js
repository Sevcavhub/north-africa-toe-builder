...
const missingGuns = [
    ...
    },
    {
        ...
        nation: 'Italian',
        caliber_mm: 149,  // Actually 149.1mm
...
async function addMissingGuns() {
    return new Promise((resolve, reject) => {
        ...
        console.log('\n' + '='.repeat(80));
        console.log('ADDING MISSING GUNS TO DATABASE');
        console.log('='.repeat(80));
        console.log(`\nAdding ${missingGuns.length} guns identified during Phase 5 Equipment Matching...\n`);
        ...
        db.serialize(() => {
            missingGuns.forEach((gun, index) => {
                // Check if gun already exists
                db.get('SELECT gun_id FROM guns WHERE gun_id = ?', [gun.gun_id], (err, row) => {
                    if (err) {
                        console.error(`[ERROR] Failed to check for ${gun.name}:`, err.message);
                        errors++;
                        return;
                    }

                    if (row) {
                        console.log(`[SKIP] ${gun.name} (${gun.gun_id}) - already exists`);
                        skipped++;
                    } else {
                        // Insert gun
                        ...
                        stmt.run(
                            ...
                            function(err) {
                                if (err) {
                                    console.error(`[ERROR] Failed to add ${gun.name}:`, err.message);
                                    errors++;
                                } else {
                                    console.log(`[OK] Added ${gun.name} (${gun.gun_id})`);
                                    console.log(`     Nation: ${gun.nation}, Caliber: ${gun.caliber_mm}mm, Type: ${gun.gun_type}`);
                                    added++;
                                }
                            }
                        );
                    ...
                    // If this is the last gun, print summary and close
                    if (index === missingGuns.length - 1) {
                        setTimeout(() => {
                            console.log('\n' + '='.repeat(80));
                            console.log('SUMMARY');
                            console.log('='.repeat(80));
                            console.log(`Guns added: ${added}`);
                            console.log(`Guns skipped (already exist): ${skipped}`);
...
if (require.main === module) {
    addMissingGuns()
        .then(() => process.exit(0))
        .catch((err) => {
            console.error('Failed to add missing guns:', err);
            process.exit(1);
        });
}

module.exports = { addMissingGuns };
