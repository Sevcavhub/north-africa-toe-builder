...
- ✅ **V6.1 Datacard Generator** (November 19, 2025): `generate_book_datacards_v6_1.py`
  ...
  - Interactive web builder: `interactive_datacard_server.py` + `interactive_datacard_builder.html`
...
#### **Phase 9B: BattleGroup Book Generation** ⏸️ **ON HOLD - Reference Data Quality Recovery**
...
**Status**: ⏸️ **ON HOLD** - Reference data quality issues discovered. Manual re-extraction of clean data in progress.

**Data Quality Discovery Date**: November 2025
**Current Focus**: Manual extraction of clean reference data (Canada's Crucible ✅ complete, British DataCards ⏳ in progress)
**Reason for Hold**: Scraped reference data from BattleGroup supplements contained errors/inconsistencies. All reverse-engineered calculations (armor conversion, penetration, movement, HE effectiveness) assumed incorrect. Clean reference data required before calculations can be validated and books completed.
...
**Phase 9B Status Summary**:
- **Infrastructure Built**: Database schema, conversion tools, book generation framework ✅
- **Content Created**: Historical chapters, scenarios, tactical templates, appendices (28,983 lines) ✅
- **Blocked**: Equipment datacards require clean reference data before publication
- **In Progress**: Manual extraction of clean reference data (2 of ~17 tasks complete)
- **Next Steps**: Validate conversion formulas with clean data, regenerate equipment stats, complete books

**📊 Data Quality Discovery & Recovery Plan** (November 2025):

**The Problem**:
After building conversion tools and generating initial book content, quality review revealed that scraped reference data from BattleGroup supplements/datacards/dispatches contained errors and inconsistencies. Since all conversion formulas (armor, penetration, movement, HE) were reverse-engineered FROM this flawed reference data, the calculated values for equipment are assumed incorrect.

**Impact**:
- Equipment datacards: Stats may be inaccurate (can't publish with uncertain values)
- Conversion formulas: Need validation against clean reference data
- Books: Infrastructure and non-equipment content (chapters, scenarios, appendices) are fine

**Recovery Strategy**:
1. ✅ **Canada's Crucible** (COMPLETE): Manual extraction of clean baseline reference data
   - 84 vehicles, 26 guns, 5 aircraft, 105 army list units
   - Provides clean validation dataset
2. ⏳ **British DataCards** (IN PROGRESS): User manually filling CSV templates
   - 77 vehicles, 15 guns, 6 aircraft identified via OCR
   - Clean data entry ongoing
3. 📋 **Remaining Extractions** (~15 tasks): Reduced scope approach
   - Original plan: Extract ALL supplements comprehensively (17+ sources)
   - **New plan**: Extract just enough samples to validate conversion formulas
   - Full extraction = "nice to have later" for personal use, not required for MVP
4. **Validation & Regeneration**: Once clean data available
...
- [x] Datacards match official format layout ✅ **V6.1 (Nov 19, 2025)**
- [x] V6.1 datacard format with weapon fallback system (255+ datacards across 12 books) ✅
- [x] Interactive web builder deployed with V6.1 generator (http://localhost:5000) ✅
